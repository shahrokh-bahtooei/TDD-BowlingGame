...
class Game(object):
    ...
    def _is_2nd_roll_in_frame(self):
        return (self._current_roll - self._count_strikes()) % 2 == 1

    def _count_strikes(self):
        strikes_count = 0
        for roll_num in range(0, self._current_roll):
            if self._rolls[roll_num] == 10:
                strikes_count += 1
        return strikes_count

    def _more_than_10_pins_are_rolled_in_this_frame(self, pins):
        return self._rolls[self._current_roll - 1] + pins > 10
    ...
    def _rolled_more_than_21_times(self):
        return self._current_roll > 20

    def _raise_error_if_rolled_after_2_non_bonus_rolls_in_10th_frame(self):
        if self._is_3rd_roll_of_10th_frame():
            has_bonus_for_strike = self._is_strike(self._current_roll - 2)
            has_bonus_for_spare = self._is_spare(self._current_roll - 2)
    ...
    def _is_3rd_roll_of_10th_frame(self):
        ...
        while frame < 10 and first_in_frame + 2 < self._current_roll:
        ...
        return frame == 10 and first_in_frame + 2 == self._current_roll
